  ...
  return (
    ...
    >
      ...
      <div className="w-full px-6 sm:px-12 lg:px-24 pt-20 md:pt-24 pb-12">
        ...
        <motion.div variants={fadeInUp} className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <h1 className="text-5xl sm:text-7xl lg:text-8xl font-black text-[#0F172A] uppercase leading-[0.9] tracking-tighter mt-2">
              EXPERT <span className="text-[#FF5722]">ISE</span>
            </h1>
          </div>
          <p className="text-slate-500 text-xs sm:text-sm font-bold uppercase tracking-widest max-w-[250px] border-l-2 border-[#FF5722] pl-4 mb-2">
          ...
          {verticals.map((v, i) => (
            ...
            >
              ...
              <div className="h-1/2 overflow-hidden relative">
                ...
                <div className="absolute top-4 left-4 bg-white p-2 text-[#FF5722] shadow-lg">
                  {v.icon}
                </div>
              </div>
              ...
              <div className="h-1/2 p-6 flex flex-col justify-between bg-white relative">
                <div>
                  <h3 className="text-2xl font-black uppercase tracking-tighter text-[#0F172A] mb-3">{v.title}</h3>
                ...
                <div className="flex items-center justify-between mt-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#FF5722]">Learn More</span>
                  <div className="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center group-hover:bg-[#FF5722] group-hover:text-white transition-all">
                    <ArrowRight size={16} />
                  </div>
                </div>
      ...
      <motion.div
        initial={{ y: 50 }}
        animate={{ y: 0 }}
        className="w-full bg-[#0F172A] grid grid-cols-2 md:grid-cols-4 divide-x divide-white/5"
      >
        {["Infotech", "Sports", "Events", "Construction"].map((item, i) => (
          <div key={i} className="py-4 text-center">
            <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.3em] group-hover:text-[#FF5722] transition-colors">{item}</span>
          </div>
        ))}
      </motion.div>
    </motion.div>
