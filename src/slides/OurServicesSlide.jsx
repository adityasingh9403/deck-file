...
export default function OurServicesSlide() {
  ...
  return (
    ...
    >
      {/* --- SIDEBAR ACCENT --- */}
      <div className="absolute left-0 top-0 w-1.5 h-full bg-[#FF5722] z-30 hidden lg:block" />
        ...
        <div className="w-full lg:w-[40%] p-8 sm:p-12 lg:p-24 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-slate-100 bg-slate-50/30">
          ...
          <motion.div variants={fadeInUp}>
            ...
            <h1 className="text-6xl sm:text-8xl lg:text-[110px] font-black text-[#0F172A] uppercase leading-[0.8] tracking-tighter mt-4">
              SOLU<span className="text-[#FF5722]">TIONS</span>
            </h1>
          </motion.div>
          ...
          {services.map((s, i) => (
            ...
            >
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div className="w-16 h-16 bg-slate-50 flex items-center justify-center text-[#FF5722] group-hover:bg-[#FF5722] group-hover:text-white group-hover:rotate-[360deg] transition-all duration-700 rounded-2xl shadow-sm">
                  {React.cloneElement(s.icon, { size: 30, strokeWidth: 1.5 })}
                </div>
                <span className="text-5xl font-black text-slate-50 group-hover:text-[#0F172A]/5 transition-colors">0{i + 1}</span>
              </div>

              <div className="relative z-10 mt-12 sm:mt-0">
                <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter text-[#0F172A] mb-3 group-hover:translate-x-2 transition-transform duration-500">
                  {s.title}
                </h3>
              ...
              <div className="relative z-10 mt-10 flex items-center gap-3 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                <div className="h-[2px] w-8 bg-[#FF5722]" />
                <span className="text-[10px] font-black text-[#FF5722] uppercase tracking-[0.2em]">Launch Case Study</span>
                <ArrowUpRight size={14} className="text-[#FF5722]" />
              </div>

              {/* Interactive Corner Accent */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-slate-50/50 -translate-y-12 translate-x-12 rotate-45 group-hover:bg-[#FF5722]/10 transition-colors" />
            </motion.div>
          ))}
        </div>
      </div>
