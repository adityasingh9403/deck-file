...
export default function HowWeDoItSlide() {
  ...
  return (
    <motion.div 
      ...
      animate="animate"
      exit="exit"
      className="relative min-h-screen w-full bg-[#FBFBFB] text-[#0F172A] overflow-x-hidden flex flex-col"
    >
      {/* --- BACKGROUND ACCENT (Laptop only) --- */}
      <div className="absolute top-0 right-0 w-1/3 h-full bg-[#FF5722]/[0.02] -skew-x-12 hidden lg:block" />
      ...
      <div className="pt-20 md:pt-24 px-6 sm:px-12 lg:px-24">
        ...
        >
          ...
          <h1 className="text-5xl sm:text-7xl lg:text-[100px] font-black text-[#0F172A] uppercase leading-[0.85] tracking-tighter mt-2">
            METHO <span className="text-[#FF5722]">DOLOGY</span>
          </h1>
        </motion.div>
      </div>
      ...
      <div className="flex-grow flex items-center px-6 sm:px-12 lg:px-24 py-12 lg:py-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full relative">
          ...
          {methods.map((step, i) => (
            ...
            >
              <div className="bg-white p-8 lg:p-10 border border-slate-100 shadow-sm hover:shadow-2xl hover:border-[#FF5722]/20 transition-all duration-500 rounded-2xl group-hover:-translate-y-4">
                
                {/* Phase Badge */}
                <div className="flex justify-between items-center mb-8">
                  <span className="text-[10px] font-black text-[#FF5722] bg-orange-50 px-3 py-1 rounded-full tracking-widest">
                    {step.tag}
                  </span>
                ...
                {/* Icon Container */}
                <div className="w-14 h-14 bg-[#0F172A] text-white flex items-center justify-center rounded-xl mb-6 shadow-lg group-hover:bg-[#FF5722] transition-colors duration-500">
                  {React.cloneElement(step.icon, { size: 28 })}
                </div>

                {/* Content */}
                <h3 className="text-xl font-black uppercase tracking-tighter text-[#0F172A] mb-3 group-hover:text-[#FF5722] transition-colors">
                  {step.title}
                </h3>
                ...
                <div className="mt-8 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                  <ArrowRight size={20} className="text-[#FF5722]" />
                </div>
              </div>

              {/* Step Connection Arrow (Laptop Only) */}
              {i < 3 && (
                <div className="absolute top-1/2 -right-4 translate-y-[-50%] text-slate-200 hidden lg:block group-hover:text-[#FF5722] transition-colors">
                   <ArrowRight size={24} />
                </div>
              )}
            </motion.div>
